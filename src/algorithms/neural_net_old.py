...
class neural_net:
    ...
    def calc_min_avg_border_dist(self, dataset, subsample=None):
        ...
        for inst_batch in data_loader:
            for instance in inst_batch:
                subfcts = self.get_neuron_border_subFcts(self.module, instance)
                dists = sorted(self.get_dists_from_border_subFcts(instance, subfcts))
    ...
    def get_top_k_closest_neurons(self, instance, k=1, dists=True):
        ...
        # neural_not_mod
        subfcts = self.get_neuron_border_subFcts(
            self.module, instance, subfct_inds=True
        )
        dists = self.get_dists_from_border_subFcts(instance, subfcts, signed_dists=True)
    ...
    def push_closest_fctborders_set(
        self, X: pd.DataFrame, cond_fct, bias_shift, subset=None
    ):
        ...
        for inst_batch in data_loader:
            for inst in inst_batch:
                ...
                pushed_samples += self.push_closest_fctborder_inst(
                    inst, bias_shift, condition
                )
                ctr += 1
        return pushed_samples

    def push_closest_fctborder_inst(
        self, instance, bias_shift, condition=None, advanced=True
    ):
        push_neurons_dists = self.get_top_k_closest_neurons(instance, k=1, dists=True)
        ...
        for neuron_ind, dist in push_neurons_dists:
            with torch.no_grad():
                ...
                if self.check_push_condition(condition, instance, dist):
                    ...
                    neural_net[neuron_ind[0]].bias[neuron_ind[1]] += (
                        bias_shift - 1
                    ) * (dist * normalized_weight)
                    if advanced:
                        self.adjust_followup_bias(
                            neuron_ind, dist, bias_shift, normalized_weight, neural_net
                        )
                    return 1
                else:
                    return 0

    def adjust_followup_bias(
        self, neuron_ind, dist, bias_shift, normalized_weight, neural_net
    ):
        with torch.no_grad():
            ...
            spread_layer_weights = neural_net[spread_layer].weight[:, neuron_ind[1]]
            spread_layer_weights_sum = sum(abs(spread_layer_weights))
            for tar_bias_ind, tar_bias in enumerate(neural_net[spread_layer].bias):
                cur_weight = neural_net[spread_layer].weight[
                    tar_bias_ind, neuron_ind[1]
                ]
                neural_net[spread_layer].bias[tar_bias_ind] = (
                    tar_bias - (cur_weight / spread_layer_weights_sum) * shift_val
                )
    ...
    def check_push_condition(self, condition, instance, dist):
        ...
        if isinstance(condition, np.float32):
            ...
            for sample_ind in range(condition.shape[0]):
                cond_fct = self.get_lin_subfct(self.module, condition[sample_ind])
    ...
    def get_interm_pre_relu_linFct(self, neural_net_mod, instance):
        ...
        for layer_ind in linLayers[:-1]:
            linSubFcts.append(self.get_lin_subfct(neural_net_mod, instance, layer_ind))
        return linSubFcts
    ...
    def get_neuron_border_subFcts(self, neural_net_mod, instance, subfct_inds=False):
        linSubFcts = self.get_interm_pre_relu_linFct(neural_net_mod, instance)
        ...
        for layer_ind, layer in zip(lin_layers, linSubFcts):
            if subfct_inds:
                subfcts = self.get_neuron_subFcts_from_layer(layer, neuron_inds=True)
                subfcts_layer = list(zip(len(subfcts) * [layer_ind], subfcts))
                subfcts_format = [((x[0], x[1][0]), x[1][1]) for x in subfcts_layer]
    ...
    def get_dists_from_border_subFcts(
        self, instance, border_subfcts, signed_dists=False
    ):
        ...
        if isinstance(border_subfcts, dict):
            ...
            for key, neuron in border_subfcts.items():
                ...
                dist_to_border = (
                    np.matmul(neuron[0], instance) + neuron[1]
    ...
    def get_neuron_subFcts_from_layer(self, layer_subfunction, neuron_inds=False):
        neuron_subFcts = []
        if neuron_inds:
