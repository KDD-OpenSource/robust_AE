...
class dataset:
    def __init__(self, name: str, file_path: str, subsample: int = None,
            scale=True, scale_type = 'MinMax', scale_min = 0, scale_max = 1):
        self.name = name
    ...
    def create(self):
        ...
        if self.label_col_train is None:
            self.train_labels = pd.Series(0,
                    index=dataset_train.index).astype(np.float32)
        else:
            self.train_labels = dataset_train.loc[
                    :,self.label_col_train].astype( np.float32)
