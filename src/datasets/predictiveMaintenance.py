...
class predictiveMaintenance(dataset):
    ...
    def create(self):
        ...
        predMaintenance_data.drop(['UDI', 'Product ID', 'Machine failure', 'TWF', 'HDF',
            'PWF', 'OSF', 'RNF'], inplace=True, axis=1)
        ...
        if self.num_samples != -1:
            ...
            # self.num_samples
            predMaintenance_data = predMaintenance_data.iloc[rand_ind:
                    rand_ind+self.num_samples]
        ...
        anomalies = np.random.uniform(low=min_per_col, high=max_per_col, size=(self.num_anomalies,
            time_windows.shape[1]))
    ...
    def create_time_windows(self, data: pd.DataFrame):
        ...
        for time_ind in range(data_values.shape[0]-self.window_size+1):
            window_values = data_values[time_ind: time_ind +
                    self.window_size].transpose()
            flattened_window = window_values.flatten()
            result = pd.concat([result,
                pd.DataFrame(flattened_window).transpose()], axis=0)
