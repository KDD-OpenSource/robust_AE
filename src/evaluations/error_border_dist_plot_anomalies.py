...
class error_border_dist_plot_anomalies:
    ...
    def evaluate(self, dataset, algorithm):
        ...
        sample_top_k_dist_pairs = algorithm.assign_top_k_border_dists(
                algorithm.module, dataset.data(), 5)
        sample_error_pairs = algorithm.assign_errors(algorithm.module,
                dataset.data())
        sample_dist_df = pd.DataFrame(sample_dist_pairs, columns =
                ['sample_dist', 'dist'])
        sample_error_df = pd.DataFrame(sample_error_pairs, columns =
                ['sample_error', 'error'])
        sample_top_k_dist_df = pd.DataFrame(sample_top_k_dist_pairs, columns =
                ['sample_top_k_dist', 'top_k_dist'])
        merged_df = sample_dist_df.merge(sample_error_df, left_index=True,
                right_index=True)
        merged_tot_df = merged_df.merge(sample_top_k_dist_df, left_index =
                True, right_index=True)
        # change by adding from self.labels
        merged_tot_df['anomaly_label'] = dataset.labels
        #merged_tot_df.iloc[-dataset.anomalies.shape[0]:,-1] = 1
        merged_tot_df['checksum'] = (merged_df['sample_dist'] -
                merged_df['sample_error'])
        if merged_tot_df['checksum'].sum().sum() > 0.001:
            print("""Your indices do not overlap. Hence error and dist are not
        ...
        df_sorted = merged_tot_df.sort_values(by=['anomaly_label','dist_float'],ascending =
                False)
        ...
        for color in df_sorted['anomaly_label'].unique():
            ...
            plt.semilogy(df_subset.index, df_subset['dist'].values, color =
                    rgba_color)
            plt.semilogy(df_subset.index, df_subset['error'].values, color = 'red')
            plt.semilogy(df_subset.index, df_subset['top_k_dist'].values, color
                    = 'blue')
