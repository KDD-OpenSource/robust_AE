...
class marabou_robust:
    def __init__(
        self, eval_inst: evaluation, name: str = "marabou_robust",
        num_eps_steps=100, delta = 0.1,
    ):
        self.name = name
    ...
    def evaluate(self, dataset, algorithm):
        ...
        for key in label_means.keys():
            ...
            self.plot_and_save(result_dict, key, solution, network,
                    input_sample, output_sample, eps, tot_time,
    ...
    def plot_and_save(self, result_dict, key, solution, network, input_sample,
            output_sample, eps, tot_time, tot_solution_stats, collapsing):
        ...
        if solution is not None:
            ...
            diff_output_max = abs(np.array(extr_solution[1]) -
                    output_sample.values[0]).max()
            diff_input_output_sample = abs(input_sample.values[0] -
                    output_sample.values[0]).max().astype(np.float64)
           #     fig, f"marabou_robust_different_pairs_label_{key}")
            ...
            ax[0].set_title(f'''L_infty dist between x and y is at most {eps}
            ...
            ax[1].set_title(f'''L_infty dist between f(x) and f(y) is at
                    least {self.delta} (real: {diff_output_max})''')
            ax[1].legend()
            ax[1].set_ylim(-1.25,1.25)
            self.evaluation.save_figure(
                fig, f"marabou_robust_same_pairs_label_{key}")
            result_dict['label_'+str(key)] = {'calc_time': tot_time, 'robustness': eps,
                    'delta': self.delta, 'real_robustness': diff_input,
    ...
    def binary_search_eps(self, eps, delta, accuracy, network, input_sample,
            output_sample):
        solution = None
        ...
        while not found_closest_eps:
            ...
            summed_solution_stats = add_solution_stats(summed_solution_stats,
                    solution_stats)
    ...
    def binary_search_delta(self, network, input_sample, output_sample, delta_accuracy):
        ...
        while not found_real_delta:
            ...
            solution, eps, summed_solution_stats = self.binary_search_eps(eps,
                    delta, accuracy, network, input_sample, output_sample)
