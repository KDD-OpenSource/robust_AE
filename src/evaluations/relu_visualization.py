...
class relu_visualization:
    def __init__(
        self, eval_inst: evaluation, name: str = "relu_visualization", num_points=20000
    ):
        self.name = name
    ...
    def evaluate(self, dataset, algorithm):
        ...
        custom_ds = np.random.uniform([-1,-1], [1,1],
                size=(10000,2)).astype(np.float32)
...
#        )
        ...
        for column in plot_data.columns[2:]:
            ...
            plt.scatter(plot_data['0_0'], plot_data['0_1'],
                    c=plot_data[column], alpha=0.5)
            self.evaluation.save_figure(fig, f"relu_vis_{column}")
            plt.close("all")
