...
class ad_box_creator(evaluation):
    def __init__(self, name: str = "ad_box_creator"):
        self.name = name
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        anomaly_scores_normal = pd.DataFrame(
            algorithm.calc_anomalyScores(algorithm.module, test_data),
        ...
        test_data_score_norm = test_data[
            test_data_scores["anomaly_score"] < norm_quantile
        ]
        test_data_score_anom = test_data[
            test_data_scores["anomaly_score"] > anom_quantile
        ]
        ...
        self.save_csv(run_inst, norm_data.transpose(), "norm_data")
        self.save_csv(run_inst, anom_data.transpose(), "anom_data")
        norm_data_pred = algorithm.predict(norm_data)
        anom_data_pred = algorithm.predict(anom_data)
        self.save_csv(run_inst, norm_data_pred.transpose(), "norm_data_pred")
        self.save_csv(run_inst, anom_data_pred.transpose(), "anom_data_pred")
        anom_quantile_dict = {"anom_quantile": anom_quantile}
        self.run_inst.save_json(anom_quantile_dict, "anom_quantile")

    def save_bounds(self, bounds, name, subfolder=None):
        # bounds is list of list
        if subfolder:
            dir_path = os.path.join(self.run_inst.run_folder, subfolder)
            ...
            file_path = os.path.join(self.run_inst.run_folder, name)
