...
class marabou_ens_normal_rob_ae(evaluation):
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        for _ in range(20):
            ...
            marabou_options = Marabou.createOptions(
                timeoutInSeconds=600, verbosity=2, numWorkers=1
            )
                ...
                for onnx_path in test_model_folders:
                    res.append(
                        self.calc_largest_error(
                            normal_point, onnx_path, eps, model_info
                        )
                    )
                    results = res[0]
    ...
    def get_test_model_folders(self, simon_folder, num_folders=None, part_model=None):
