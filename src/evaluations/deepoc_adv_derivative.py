...
class deepoc_adv_derivative(evaluation):
    def __init__(
        self,
    ...
    def find_adv_attack(self, input_sample, algorithm):
        ...
        adv_sample = self.find_adv_sample(
            input_sample, algorithm, bin_search_start, acc
        )
    ...
    def calc_cosine_sim(self, input_sample, adv_attack, algorithm):
        ...
        adv_attack_im = (
            (algorithm.module(torch.tensor(adv_attack.astype(np.float32)))[0])
            .detach()
            .numpy()
        )
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        for i, input_sample in enumerate(samples):
            ...
            adv_attack, L2_dist, L_infty_dist = self.find_adv_attack(
                input_sample, algorithm
            )
            ...
            check_anomaly = algorithm.check_anomalous(
                pd.DataFrame(adv_attack).transpose()
            ).tolist()
            result_dict[i] = {
        ...
        self.save_json(run_inst, result_dict, "deepoc_adv_derivative")
        self.save_json(run_inst, result_dict_stats, "deepoc_adv_derivative_stats")
