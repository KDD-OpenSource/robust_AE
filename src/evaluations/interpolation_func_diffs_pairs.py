...
class interpolation_func_diffs_pairs:
    def __init__(
        self, eval_inst: evaluation, name: str = "interpolation_func_diffs"
    ):
        self.name = name
...
#            label_ind = dataset.labels[dataset.labels == label].sample(1).index
#            label_inds.append(label_ind)
#            label_inst = dataset.data().loc[label_ind]
#            import pdb; pdb.set_trace()
#            insts = pd.concat([insts, label_mean], axis=0)
#        for label_ind1, label_ind2 in list(combinations(label_inds,2)):
#            label1 = dataset.labels[label_ind1].item()
#            label2 = dataset.labels[label_ind2].item()
...
#            self.evaluate_inst_pair(dataset, algorithm, inst_pair, subfolder)
        ...
        label_mean_tuples = list(zip(labels, means))
    ...
    def evaluate_inst_pair(self, dataset, algorithm, insts, subfolder):
        ...
        #for row_ind in range(0,interp_func_df.shape[0],10):
            ...
            axs[1].plot(lin_func_applied_no_bias,
                    label='lin_func_applied_without_bias')
            ...
            fig.suptitle(f"""Relevance of Orig: {relevance_orig}, Relevance
        ...
        plt.plot(interp_func_df.index, interp_func_df['seq_func_dist'].values, color =
                'b', label='dist_sum')
        #import pdb; pdb.set_trace()
