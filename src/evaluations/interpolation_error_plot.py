...
class interpolation_error_plot:
    def __init__(
        self, eval_inst: evaluation, name: str = "interpolation_error_plot"
    ):
        self.name = name
...
#            self.evaluate_inst_pair(dataset, algorithm, inst_pair, subfolder)
    ...
    def evaluate_inst_pair(self, dataset, algorithm, insts, subfolder):
        ...
        interp_errors = np.sqrt(((interpolations -
            reconstructions)**2).sum(axis=1).values)
        plt.plot(interp_errors, label='interpolation_errors')
        plt.legend()
        self.evaluation.save_figure(fig, "interpolation_errors",
                subfolder=subfolder)
        self.evaluation.save_csv(pd.DataFrame(interp_errors,
            columns=['interp_errors']), "interpolation_errors",
                subfolder=subfolder)
        plt.close("all")
