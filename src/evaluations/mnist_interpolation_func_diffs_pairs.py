...
class mnist_interpolation_func_diffs_pairs:
    def __init__(
        self, eval_inst: evaluation,
        name: str = "mnist_interpolation_func_diffs_pairs"
    ):
        self.name = name
    ...
    def evaluate(self, dataset, algorithm):
        ...
        #NOTE: dataset.labels['label'] will have to be replaced (as it is now a series)
        for label in dataset.labels['label'].unique():
            sample_index = dataset.labels[dataset.labels['label'] ==
                    label].sample(1).index
            sample = dataset.data().iloc[sample_index]
            samples = pd.concat([samples, sample], axis=0)

        for digit1, digit2 in list(combinations(list(samples.index), 2)):
            insts = dataset.data().loc[[digit1, digit2]]

            label_from = dataset.labels.loc[digit1]['label']
            label_to = dataset.labels.loc[digit2]['label']

            interpolations = interpolate(insts, 1000)
            interp_func_pairs = algorithm.assign_lin_subfcts(
                    algorithm.module, interpolations)
            interp_func_df = pd.DataFrame(interp_func_pairs, columns = ['sample',
                'function'])
            ...
            #for row_ind in range(0,interp_func_df.shape[0],10):
                ...
                interp_func_df.loc[
                        row_ind, 'sign_sum_weighted'] = dist_sign_sum_weighted
                ...
                fig.suptitle(f"""Relevance of image: {relevance_image}, Relevance
            ...
            axs[0].plot(interp_func_df.index, interp_func_df['seq_func_dist'].values, color =
                    'b', label='dist_sum')
            axs[0].legend()
            axs[1].plot(interp_func_df.index,
                    interp_func_df['seq_func_dist_bias'].values, color = 'r',
                    label='dist_bias')
            axs[1].legend()
            axs[2].plot(interp_func_df.index, interp_func_df['seq_func_dist_mat'].values, color =
                    'g', label='dist_mat')
            axs[2].legend()
            axs[3].plot(interp_func_df.index, interp_func_df['sign_sum'].values, color =
                    'm', label='sign_sum')
            axs[3].legend()
            axs[4].plot(interp_func_df.index, interp_func_df['sign_sum_weighted'].values, color =
...
def interpolate(insts, res_elems):
    ...
    for i in range(1,interp_steps+1):
        interp_inst = ((interp_steps-i)/interp_steps)*inst1 + (i/interp_steps) * inst2 
