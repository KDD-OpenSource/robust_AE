...
class mse_test:
    ...
    def evaluate(self, dataset, algorithm):
        ...
        for label in dataset.test_labels.unique():
            ...
            mse_label = np.sqrt(
                    ((label_data_pred - label_data)**2).sum(axis=1)
                    ).sum()
        ...
        tot_error = np.sqrt(
                    ((algorithm.predict(dataset.test_data()) -
                        dataset.test_data())**2).sum(axis=1)
                    ).sum()
        result_dict = {}
        label_mean_tuples = list(zip(test_labels, list(map(lambda x:
            x.astype(np.float64), label_error))))
