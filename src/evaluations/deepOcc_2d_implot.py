...
class deepOcc_2d_implot(evaluation):
    def __init__(self, name: str = "deepOcc_2d_implot"):
        self.name = name
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        circle = plt.Circle(
            (algorithm.center[0], algorithm.center[1]),
            algorithm.anom_radius,
            edgecolor="blue",
            fill=False,
        )
        ax[0].add_artist(circle)
        # self.save_figure(run_inst, fig, "deepOcc_2d_implot")
        ...
        ax[1].scatter(
            pred_normal.iloc[:, 0], pred_normal.iloc[:, 1], color=cmap(labels_normal)
        )
        ...
        self.save_figure(run_inst, fig, "deepOcc_2d_implot_normal")
