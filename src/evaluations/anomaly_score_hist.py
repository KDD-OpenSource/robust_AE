...
class anomaly_score_hist(evaluation):
    def __init__(self, name: str = "anomaly_score_hist"):
        self.name = name
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        self.save_figure(run_inst, fig, f"anomaly_hist_{n_bins}")
        # self.run_inst.save_json(result_dict, "label_error")
        # result_dict_tot = {}
        # result_dict_tot["total_error_test_ds"] = tot_error.astype(np.float64)
        # self.run_inst.save_json(result_dict_tot, "tot_error")
