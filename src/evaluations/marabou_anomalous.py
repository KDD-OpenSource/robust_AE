...
class marabou_anomalous:
    def __init__(
        self, eval_inst: evaluation, name: str = "marabou_anomalous",
        ...
        for num_anomaly in range(5):
            ...
            while not found_closest_non_anomaly:
                ...
                for ind in range(numInputVars):
                    loaded_network.setLowerBound(ind, input_sample.values[0][ind] - delta)
                    loaded_network.setUpperBound(ind, input_sample.values[0][ind] + delta)
                network_solution, stats = maraboupy.MarabouCore.solve(loaded_network, options)
            ...
            if solution is not None:
                ...
                ax[0].set_title(f'''L_infty dist is at most {delta} (up to
                ...
                ax[1].set_title(f'''L_infty dist is at most {self.eps}, thus we
                        have a non_anomaly''')
                ax[1].legend()
                self.evaluation.save_figure(
                    fig, f"marabou_anomalous_{num_anomaly}")
                result_dict['anomaly'] = {'calc_time': tot_time,
            ...
            self.evaluation.save_json(result_dict,
                    f'results_marabou_anomalous_{num_anomaly}')
