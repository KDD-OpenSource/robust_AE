...
class marabou_svdd_normal_rob(evaluation):
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        for _ in range(20):
            ...
            largest_error_vals = dict(
                map(lambda x: (x["id"], x["largest_error"]), results)
            )
            times = dict(map(lambda x: (x["id"], x["calc_time"]), results))
            max_dists = dict(map(lambda x: (x["id"], x["max_dist"]), results))
            ratios = dict(map(lambda x: (x["id"], x["error_ratio"]), results))
            robustness_vals = dict(map(lambda x: (x["id"], x["robustness"]), results))
            end_time = time.time()
            # add res_dict
            sample_res_dict = {}
        ...
        while delta_change > accuracy:
            ...
            for outputInd in range(numOutputVars):
                ...
                if len(network_solution[0]) > 0:
                    ...
                    l_infty_error = abs(
                        np.array(extract_solution_point(current_solution, network)[1])
                        - np.array(center)
                    ).max()
                    l_2_error = np.sqrt(
                        (
