...
class marabou_ens_normal_rob(evaluation):
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        if "submodels.json" in os.listdir(
            os.path.join(simon_folder, "models")
        ) and re.match(".*_\d_.*", self.cfg.ctx):
            with open(
                os.path.join(simon_folder, "models", "submodels.json"), "r"
            ) as jsonfile:
        ...
        with open(
            os.path.join(simon_folder, "models", "border.json"), "r"
        ) as json_file:
        ...
        for _ in range(20):
            ...
            for i in adv_df.columns:
                ...
                other_models = tot_models - adv_df[i].isnull().sum() - mode_models
            ...
            self.save_json(
                run_inst, largest_error_dict, f"largest_error_dict_{norm_ind}"
            )
            self.save_json(run_inst, eps_res_dict, f"eps_res_dict_{norm_ind}")
            self.save_json(run_inst, mode_dict, f"mode_dict_{norm_ind}")
    ...
    def calc_largest_error(self, normal_point, onnx_path, eps, model_info, q_values):
        ...
        model_info.append(
            self.get_model_info(
                onnx_path, q_values, numInputVars, normal_point.shape[1]
            )
        )
    ...
    def get_test_model_folders(
        self, simon_folder, q_values, num_folders=None, part_model=None
    ):
        ...
        if num_folders is not None:
            ...
            while len(rand_models) < num_folders:
                ...
                if rand_model not in rand_models and q_values[rand_model] >= min_q:
