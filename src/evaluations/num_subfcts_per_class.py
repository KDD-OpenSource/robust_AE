...
class num_subfcts_per_class(evaluation):
    def __init__(self, name: str = "num_subfcts_per_class"):
        self.name = name
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        for label in test_labels.unique():
            ...
            label_subfcts = len(
                algorithm.count_lin_subfcts(algorithm.module, label_data)
            )
        ...
        for label in normal_labels:
            ...
            noisy_subfcts = len(
                algorithm.count_lin_subfcts(algorithm.module, noisy_label_data)
            )
            result_dict[f"Noisy Testdata: {str(label)}"] = noisy_subfcts

        self.save_json(run_inst, result_dict, "num_subfcts_per_class")
