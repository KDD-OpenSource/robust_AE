...
class parallel_plot_same_subfcts(evaluation):
    def __init__(self, name: str = "parallel_plot_same_subfcts"):
        self.name = name
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        for ind, label in enumerate(normal_labels):
            ...
            min_avg_border_dist = algorithm.calc_min_avg_border_dist(
                label_data, subsample=min(label_data.shape[0], 1000)
            )

            inst_fct_pairs = algorithm.assign_lin_subfcts(algorithm.module, label_data)
            inst_fct_pairs_data = list(
                map(lambda x: (np.array(x[0].tolist()), x[1]), inst_fct_pairs)
            )
            inst_fct_df = pd.DataFrame(inst_fct_pairs_data, columns=["insts", "fct"])
            fct_means = inst_fct_df.groupby(["fct"], sort=False).insts.apply(np.mean)
            ...
            if "scales" in dataset.__dict__.keys():
                ax[ind].set_title(
                    f"""Num_fcts: {len(fct_means)}, label: {label},
        ...
        self.save_figure(run_inst, fig, "parallel_plot_subfct_means")
