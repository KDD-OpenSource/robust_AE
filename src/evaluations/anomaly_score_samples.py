...
class anomaly_score_samples(evaluation):
    def __init__(self, name: str = "anomaly_score_samples"):
        self.name = name
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        noise_data = pd.DataFrame(
            np.random.uniform(-1, 1, size=normal_data.shape).astype(np.float32)
        )

        anomaly_eps = 5.0
        anomaly_base_samples = test_data.sample(num_samples)
        anomaly_vects = np.random.uniform(
            -1, 1, size=anomaly_base_samples.shape
        ).astype(np.float32)
        anomaly_base_vect_sum = anomaly_base_samples + anomaly_vects
        anomaly_result_unscaled = (
            anomaly_base_samples[
                (anomaly_base_vect_sum < -1) | (anomaly_base_vect_sum > 1)
            ]
            - anomaly_vects
        ).fillna(anomaly_base_vect_sum)
        ...
        for image, anom_score in zip(normal_data.values, anomaly_scores_normal.values):
        ...
        ctr = 0
        for image, anom_score in zip(noise_data.values, anomaly_scores_noise.values):
            fig, ax = plt.subplots(nrows=1, ncols=1, figsize=[10, 10])
            image_resh = image.reshape(im_dim_size, im_dim_size)
            ax.imshow(image_resh, cmap="gray")
            ax.set_title(f"Anomaly_score: {anom_score}")
            self.save_figure(run_inst, fig, f"plot_noise_sample_{ctr}")
            plt.close("all")
            ctr += 1

        ctr = 0
        for image, anom_score in zip(
            anomalous_samples.values, anomaly_scores_anom.values
        ):
            fig, ax = plt.subplots(nrows=1, ncols=1, figsize=[10, 10])
            image_resh = image.reshape(im_dim_size, im_dim_size)
            ax.imshow(image_resh, cmap="gray")
            ax.set_title(f"Anomaly_score: {anom_score}")
            self.save_figure(run_inst, fig, f"plot_anom_sample_{ctr}")
            plt.close("all")
            ctr += 1
