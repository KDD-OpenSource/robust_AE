...
class fct_change_by_border_dist_qual:
    def __init__(
        self, eval_inst: evaluation, name: str =
    ...
    def evaluate(self, dataset, algorithm):
        sample_dist_pairs = algorithm.assign_most_far_border_dists(algorithm.module,
                dataset.data())
        sample_dist_df = pd.DataFrame(sample_dist_pairs, columns =
                ['sample', 'dist'])
        df_sorted = sample_dist_df.sort_values(by=['dist'],ascending =
                False)
        ...
        for df_entry in largest_dist_df.values:
            ...
            lrp, relevance_bias = algorithm.lrp_ae(algorithm.module,
                    image)
            lin_func_feature_imp = algorithm.lin_func_feature_imp(algorithm.module,
                    image)
            lin_func_applied_no_bias = algorithm.apply_lin_func_without_bias(
                    algorithm.module, image
                    )
            lin_func_bias_imp = algorithm.lin_func_bias_imp(algorithm.module,
                    image)
            ...
            ac_lrp, ac_relevance_bias = algorithm.lrp_ae(algorithm.module,
                    after_cross)
            ac_lin_func_feature_imp = algorithm.lin_func_feature_imp(algorithm.module,
                    after_cross)
            ac_lin_func_applied_no_bias = algorithm.apply_lin_func_without_bias(
                    algorithm.module, after_cross
                    )
            ac_lin_func_bias_imp = algorithm.lin_func_bias_imp(algorithm.module,
                    after_cross)
            ...
            ac_lin_func_feature_imp = ac_lin_func_feature_imp.reshape(fig_side,fig_side)
            ...
            lin_fct_no_bias_diff = (lin_func_applied_no_bias -
            ...
            vmin_fct_imp = min(lin_func_feature_imp.min(), ac_lin_func_feature_imp.min())
            vmax_fct_imp = max(lin_func_feature_imp.max(), ac_lin_func_feature_imp.max())

            vmin_fct_applied_no_bias = min(lin_func_applied_no_bias.min(), ac_lin_func_applied_no_bias.min())
            vmax_fct_applied_no_bias = max(lin_func_applied_no_bias.max(),
                    ac_lin_func_applied_no_bias.max())

            vmin_bias = min(lin_func_bias_imp.min(), ac_lin_func_bias_imp.min())
            vmax_bias = max(lin_func_bias_imp.max(), ac_lin_func_bias_imp.max())


            axs[0,0].imshow(image_resh, cmap = 'gray', vmin=vmin_ac_orig,
                    vmax=vmax_ac_orig)
            axs[0,0].set_title('Orig')
            axs[0,1].imshow(reconstr_resh, cmap = 'gray', vmin=vmin_recon,
                    vmax=vmax_recon)
            axs[0,1].set_title('Reconstruction')
            axs[0,2].imshow(lrp_resh, cmap = 'gray', vmin=vmin_lrp,
                    vmax=vmax_lrp)
            axs[0,2].set_title('LRP')
            axs[0,3].imshow(lin_func_feature_imp, cmap = 'gray',
                    vmin=vmin_fct_imp,
                    vmax=vmax_fct_imp)
            axs[0,3].set_title('Lin_Func_Feature_Imp')
            axs[0,4].imshow(lin_func_applied_no_bias, cmap = 'gray',
                    vmin=vmin_fct_applied_no_bias,
                    vmax=vmax_fct_applied_no_bias)
            axs[0,4].set_title('Lin_func_applied_no_bias')
            axs[0,5].imshow(lin_func_bias_imp, cmap = 'gray', vmin=vmin_bias,
                    vmax=vmax_bias)
            axs[0,5].set_title('Lin_Func_Bias_Imp')
            axs[1,0].imshow(ac_resh, cmap = 'gray', vmin=vmin_ac_orig,
                    vmax=vmax_ac_orig)
            axs[1,0].set_title('After_cross')
            axs[1,1].imshow(ac_reconstr_resh, cmap = 'gray', vmin=vmin_recon,
                    vmax=vmax_recon)
            axs[1,1].set_title('AC_Reconstruction')
            axs[1,2].imshow(ac_lrp_resh, cmap = 'gray', vmin=vmin_lrp,
                    vmax=vmax_lrp)
            axs[1,2].set_title('AC_LRP')
            axs[1,3].imshow(ac_lin_func_feature_imp, cmap = 'gray',
                    vmin=vmin_fct_imp,
                    vmax=vmax_fct_imp)
            axs[1,3].set_title('AC_Lin_Func_Feature_Imp')
            axs[1,4].imshow(ac_lin_func_applied_no_bias, cmap = 'gray',
                    vmin=vmin_fct_applied_no_bias,
                    vmax=vmax_fct_applied_no_bias)
            axs[1,4].set_title('AC_Lin_func_applied_no_bias')
            axs[1,5].imshow(ac_lin_func_bias_imp, cmap = 'gray', vmin=vmin_bias,
                    vmax=vmax_bias)
            axs[1,5].set_title('AC_Lin_Func_Bias_Imp')
            axs[2,0].imshow(orig_ac_diff, cmap = 'gray',vmin=vmin_ac_orig,
                    vmax=vmax_ac_orig)
            axs[2,0].set_title('orig_ac_diff')
            axs[2,1].imshow(recon_diff, cmap = 'gray', vmin=vmin_recon,
                    vmax=vmax_recon)
            axs[2,1].set_title('recon_diff')
            axs[2,2].imshow(lrp_diff, cmap = 'gray', vmin=vmin_lrp,
                    vmax=vmax_lrp)
            axs[2,2].set_title('lrp_diff')
            axs[2,3].imshow(lin_fct_diff, cmap = 'gray',
                    vmin=vmin_fct_imp,
                    vmax=vmax_fct_imp)
            axs[2,3].set_title('lin_fct_diff')
            axs[2,4].imshow(lin_fct_no_bias_diff, cmap = 'gray',
                    vmin=vmin_fct_applied_no_bias,
                    vmax=vmax_fct_applied_no_bias)
            axs[2,4].set_title('AC_Lin_func_applied_no_bias_diff')
            axs[2,5].imshow(bias_fct_diff, cmap = 'gray', vmin=vmin_bias,
                    vmax=vmax_bias)
            axs[2,5].set_title('bias_fct_diff')
            self.evaluation.save_figure(fig,
                    f"plot_mnist_sample_largest_dist_withAC{ctr}")
            plt.close("all")
            ctr +=1
        ...
        for df_entry in mid_dist_df.values:
            ...
            axs[2,5].set_title('bias_fct_diff')
            self.evaluation.save_figure(fig,
                    f"plot_mnist_sample_mid_dist_withAC{ctr}")
            plt.close("all")
            ctr +=1
        ...
        for df_entry in smallest_dist_df.values:
            ...
            axs[2,5].set_title('bias_fct_diff')
            self.evaluation.save_figure(fig,
                    f"plot_mnist_sample_smallest_dist_withAC{ctr}")
            plt.close("all")
            ctr +=1
