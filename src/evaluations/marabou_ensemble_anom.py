...
class marabou_ensemble_anom(evaluation):
    def __init__(
        self,
    ...
    def evaluate(self, dataset, algorithm, run_inst):
        ...
        result_dict = {}
        for _ in range(10):
            normal_point = self.cust_scaler(
                test_data[dataset.test_labels == 7].sample(1)
            )
            simon_folder = os.path.join(
                os.getcwd(), "models", "trained_models", "simon2"
            )
            ...
            self.save_json(run_inst, largest_error_dict, f"largest_errors_{norm_ind}")
        ...
        while delta_change > 0.0000001:
            ...
            if len(network_solution[0]) > 0:
                if (
                    ...
                    or network_solution[0][outputVar] < q - delta
                ):
